 ...
 */
 ...
 predicate isFree(DataFlow::Node outgoing, DataFlow::Node incoming, Expr e, DeallocationExpr dealloc) {
   exists(Expr conv |
    ...
    //  and
    //  conv.getLocation().getFile().getAbsolutePath().matches("%/coroparse.c")
    //  and conv.getLocation().getStartLine() = 1548
    //  and
    //  conv.getLocation().getFile().getAbsolutePath().matches("%/pch.c") 
    //  conv.getLocation().getFile().getBaseName() = "pch.c"
   ) 
   and
